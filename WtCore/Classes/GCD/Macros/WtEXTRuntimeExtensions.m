...
wt_propertyAttributes *wt_copyPropertyAttributes(objc_property_t property) {
  ...
  wt_propertyAttributes *attributes = calloc(1, sizeof(wt_propertyAttributes) + typeLength + 1);
  if (!attributes) {
    fprintf(stderr, "ERROR: Could not allocate wt_propertyAttributes structure for attribute string \"%s\" for property %s\n", attrString, property_getName(property));
    return NULL;
  }
  ...
  while (next && *next == ',') {
    ...
    switch (flag) {
      ...
      case 'C':
        attributes->memoryManagementPolicy = wt_propertyMemoryManagementPolicyCopy;
        break;

      case '&':
        attributes->memoryManagementPolicy = wt_propertyMemoryManagementPolicyRetain;
        break;
